      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        >
          ...
          <h2 className="text-4xl md:text-5xl font-bold mb-6">
            <span className="text-[#003466]">نسعد</span>{' '}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16">
          ...
          >
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
              <div>
                <label className="block text-[#003466] mb-2">الاسم</label>
                <input
                  {...register('name')}
                  className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:border-[#00B272] outline-none"
              ...
              <div>
                <label className="block text-[#003466] mb-2">البريد الإلكتروني</label>
                <input
                  {...register('email')}
                  className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:border-[#00B272] outline-none"
              ...
              <div>
                <label className="block text-[#003466] mb-2">رقم الهاتف</label>
                <input
                  {...register('phone')}
                  className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:border-[#00B272] outline-none"
              ...
              <div>
                <label className="block text-[#003466] mb-2">الرسالة</label>
                <textarea
                  {...register('message')}
                  className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:border-[#00B272] outline-none h-32"
          ...
          >
            {contactInfo.map((info, index) => (
              <div key={index} className="bg-white/80 backdrop-blur-md p-6 rounded-2xl shadow-lg">
                <div className="flex items-center gap-4 mb-4">
                  <div className="bg-gradient-to-r from-[#003466] to-[#00B272] p-3 rounded-xl">
                    <info.icon className="w-6 h-6 text-white" />
                  </div>
                  <h3 className="text-xl font-bold text-[#003466]">{info.title}</h3>
                </div>
                {info.details.map((detail, idx) => (
                  <p key={idx} className="text-gray-600 mr-14">{detail}</p>
                ))}
              </div>
            ))}

            {/* Map */}
            <div className="bg-white/80 backdrop-blur-md p-4 rounded-2xl shadow-lg h-64">
              <iframe
